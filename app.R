...
#
#################################################################
header <- dashboardHeader(title = tags$a("好みのタイプ診断", href = "http://shiny.datumstudio.jp/type_decision_tree"))

sidebar <- dashboardSidebar(
	sidebarMenu(
		menuItem("リンク", tabName = "app"),
		tags$li(tags$a(href = "http://shiny.datumstudio.jp/type_decision_tree", icon("home"), span("トップページ"))),
    ...
    if (!is.null(fit$frame) && nrow(fit$frame) >= 2) {
      ...
      result_text <- "女性が好きなようです。"
		...
		output$facebook <- renderUI({
		  tags$a(href = sprintf("http://www.facebook.com/dialog/feed?app_id=392319910975259&redirect_uri=http://shiny.datumstudio.jp/type_decision_tree&link=http://shiny.datumstudio.jp/type_decision_tree&description=女性の好みをデータから診断！！&name=「%s」&picture=http://bds.datumstudio.jp/wp-content/uploads/koukoku3.jpg", result_text), 
		      target = "_blank",
		      class = "facebook-iine",
		      icon("facebook"),
		      tags$span(tags$strong("結果をシェア"))
